    ...
    for index, row in tqdm(input_excel_df.iterrows(), total=len(input_excel_df)):
        ...
        try:
            if url == 'No available file':
                ...
                if checker1:
                    input_excel_df.at[index, 'purpose'] = parse_type_1(url)
                elif checker2:
                    input_excel_df.at[index, 'purpose'] = parse_type_2(url)
                    input_excel_df.at[index, 'text'] = parse_type_2(url)
                elif checker3:
                    input_excel_df.at[index, 'purpose'] = parse_type_3(url)
                    input_excel_df.at[index, 'text'] = parse_type_3(url)
                else:
...
def parse_type_2(url):
    ...
    try:
        ...
        funding_op_purpose = soup.find(class_='col-md-4 datalabel', text='Funding Opportunity Purpose')
        print(funding_op_purpose)
